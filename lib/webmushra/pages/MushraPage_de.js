...
MushraPage.prototype.render = function (_parent) {
  ...
  // // remove start/stopp button on the top left:
  // var tdLoop1 = $(" \
  //   <td class='stopButton'> \
...
MushraPage.prototype.renderCanvas = function(_parentId) {
  ...
  // set lines here
  var xDrawingStart =   $('.scales').first().offset().left - $('#mushra_items').offset().left + $('.scales').last().width()/2;
  ...
  canvasContext.fillText(this.pageManager.getLocalizer().getFragment(this.language, 'bad'), xLetters, prevScalesHeight * 1 + YfirstLine);
